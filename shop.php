<!DOCTYPE html>
<html lang="en">
   <!--====== Start Header ======-->
   <head>
      <title>Alpha Technical Services - Shop</title>
      <meta name="description" content="Find the latest products and technologies for your engineering needs in the ATS online shop."/>
      <?php include_once('./include/head.php'); ?>
   </head>
   <!--====== End Header ======-->
   <body>
      <?php include './include/sections/preloader.php'; ?>
      <!--====== Start Header ======-->
      <?php include_once('./include/header-2.php'); ?>
      <!--====== End Header ======-->
      <!--====== Start breadcrumbs section ======-->
      <section class="breadcrumbs-section bg_cover" style="background-image: url(assets/images/bg/contact.png);">
         <div class="container">
            <div class="row">
               <div class="col-lg-5">
                  <div class="breadcrumbs-content">
                     <h1>Shop Now</h1>
                     <ul class="link">
                        <li><a href="index.html">Home</a></li>
                        <li class="active">Shop Now</li>
                     </ul>
                  </div>
               </div>
            </div>
         </div>
      </section>
      <!--====== End breadcrumbs section ======-->
      <!--====== Start Safety Policy Section ======-->
      <section class="about-area-v3 bg_cover pt-80 pb-80">
         <div class="container">
            <div class="container">
               <div class="container">
                  <h2 style="color:blue; text-align:center">Coming Soon</h2>
               </div>
            </div>
         </div>
      </section>
      <!--====== End Safety Policy Section ======-->
      <?php include_once './include/sections/contact-section.php'; ?>

      <!--====== Start Footer ======-->
      <?php include_once('./include/footer.php'); ?>
      <!--====== End Footer ======-->
      <!--====== back-to-top ======-->
      <a href="#" class="back-to-top" ><i class="flaticon-up-arrow-angle"></i></a>
      <!--====== Jquery js ======-->
      <script src="assets/js/vendor/modernizr-3.6.0.min.js"></script>
      <script src="assets/js/vendor/jquery-1.12.4.min.js"></script>
      <!--====== Bootstrap js ======-->
      <script src="assets/js/popper.min.js"></script>
      <script src="assets/js/bootstrap.min.js"></script>
      <!--====== Slick js ======-->
      <script src="assets/js/slick.min.js"></script>
      <!--====== Magnific Popup js ======-->
      <script src="assets/js/jquery.magnific-popup.min.js"></script>
      <!--====== Isotope js ======-->
      <script src="assets/js/isotope.pkgd.min.js"></script>
      <!--====== Imagesloaded js ======-->
      <script src="assets/js/imagesloaded.pkgd.min.js"></script>
      <!--====== nice-select js ======-->
      <script src="assets/js/jquery.nice-select.min.js"></script>
      <!--====== counterup js ======-->
      <script src="assets/js/jquery.counterup.min.js"></script>
      <!--====== waypoints js ======-->
      <script src="assets/js/waypoints.min.js"></script>
      <!--====== Main js ======-->
      <script src="assets/js/main.js"></script>
   </body>
</html>